    ...
    public class EmailService : IEmailService
    {
        ...
        public async Task<bool> SendConfirmationEmailAsync(User user)
        {
            try 
            {
                ...
                var emailBody = CreateConfirmationEmailBody(user.Email, token);

                // Send email using Gmail SMTP
                await SendEmailWithGmailSmtp(
                    to: user.Email, 
                    subject: "Confirm Your Email", 
                    body: emailBody //was emailBody
                );
        ...
        private string CreateConfirmationEmailBody(string email, string token)
        {
            return $@"
            <html>
            <body>
                ...
                <p>Hello,</p>
                <p>Thank you for registering. Please confirm your email by entering the token below:</p>
                <p>{token}</p>
